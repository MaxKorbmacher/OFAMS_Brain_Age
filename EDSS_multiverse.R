# Multiverse to run all reasonable LMs for EDSS
...
for (i in 1:10){ # select all possible combinations up to the order of 10 (higher up, the model will be overparameterised & computation is not possible anymore)
  vc = combn(vars,i)
  for (j in 1:ncol(vc)){
    model <- as.formula(paste0("edss_ARC~sex+Age_BL_OFAMS+", paste0(vc[,j], collapse = "+")))
...
print("Writing file.")
write.csv(result,"/Users/max/Documents/Local/MS/results/EDSS_effects.csv")
#
print("Done.")
